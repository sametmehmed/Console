namespace Codility
{
    internal class MaxCounters
    {
        public static int[] solution(int N, int[] A)
        {
            ...
            foreach (int i in A)
            {
                ...
                {
                    if (solution[i - 1] < lastMaxValue)
                        solution[i - 1] = lastMaxValue + 1;
                    else
                        solution[i - 1]++;

                    if (maxValue < solution[i - 1])
                        maxValue = solution[i - 1];
                }
            }

            for(int i = 0;i < solution.Length;i++)
            {
                if (solution[i] < lastMaxValue)
                    solution[i] = lastMaxValue;
            }
    ...
    a structure Results (in C), or a vector of integers (in C++), or a record Results (in Pascal), or an array of integers
    (in any other programming language). For example, given:
    ...
    expected worst-case time complexity is O(N+M); expected worst-case space complexity is O(N), beyond input storage 
